...
def get_possible(path, dataset_ids):
    ...
    return list(set(dataset_ids).intersection(possible_ids))
...
def get_tacotron2_datasets(path: Path, batch_size):
    ...
    random.seed(1234)
    random.shuffle(dataset_ids)
    sample_ids = dataset_ids[:5]
    sample_dataset = TextMelLoader(path, sample_ids ,text_dict, hparams)
    ...
    return train_loader, sample_loader, attn_example
